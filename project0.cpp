...
void readWithReserve(vector<double> &data) {
    ...
    cin >> size;
    data.reserve(size);
    //size of vector is whats in it; capacity is how much dynamic memory
    //Resize: if we know the size, effects both
    //Reserve: effects capacity so that it has room for pushback
    for(size_t i = 0; i < size; i++){
        double temp;
        cin >> temp;
        data.push_back(temp);
